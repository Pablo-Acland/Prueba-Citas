...
public class citasReactivaServiceImpl implements IcitasReactivaService {
    ...
    public Flux<citasDTOReactiva> cancelarCita(String id) {
        return this.IcitasReactivaRepository.findByIdPaciente(id)
                .flatMap(citas -> {
                    citas.setEstadoReservaCita("Cancelado");
                    return save(citas);
                })
                .switchIfEmpty(Mono.empty());
    }
    ...
    @Override
    public Flux<citasDTOReactiva> consultarMedicoAtendera(String id) {
        return this.IcitasReactivaRepository.findByIdPaciente(id)
                .flatMap(citas -> {
                            citasDTOReactiva Citas2 = new citasDTOReactiva();
                            Citas2.setNombreMedico(citas.getNombreMedico());
                            Citas2.setApellidosMedico(citas.getApellidosMedico());
                            return Flux.just(Citas2);
                        }
                )
                .switchIfEmpty(Mono.empty());
    }
